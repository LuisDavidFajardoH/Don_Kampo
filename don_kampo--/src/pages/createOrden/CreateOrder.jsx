...
const CreateOrder = () => {
  ...
  const fetchUserDetails = async (userId) => {
    try {
      const response = await axios.get(`https://api.donkampo.com/api/users/${userId}`);
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get("https://api.donkampo.com/api/products", {
          withCredentials: true,
        });
  ...
  const handleSubmit = async (values) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post("https://api.donkampo.com/api/orders/placeOrder", orderData);
