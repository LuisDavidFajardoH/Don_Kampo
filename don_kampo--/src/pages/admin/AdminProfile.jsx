...
const AdminProfile = () => {
  ...
  const sendProductsToAPI = async (products) => {
    try {
      for (const product of products) {
        ...
        await axios.post("http://3.22.98.109:8080/api/createproduct", formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      }
  ...
  const fetchShippingCosts = async () => {
    try {
      const response = await axios.get("http://3.22.98.109:8080/api/customer-types");
  ...
  const updateShippingCosts = async (values) => {
    ...
    try {
      await axios.put("http://3.22.98.109:8080/api/customer-types/shipping-costs", values);
  ...
  const fetchOrders = async () => {
    try {
      const response = await axios.get("http://3.22.98.109:8080/api/orders");
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      ...
      await axios.put(`http://3.22.98.109:8080/api/updatestatus/${orderId}/${newStatus}`);
  ...
  const deleteOrder = async (orderId) => {
    try {
      await axios.delete(`http://3.22.98.109:8080/api/deleteorders/${orderId}`);
  ...
  const openUserModal = async (user) => {
    try {
      const response = await axios.get(`http://3.22.98.109:8080/api/users/${user.id}`);
  ...
  const updateUserDetails = async (values) => {
    try {
      await axios.put(`http://3.22.98.109:8080/api/updateusers/${selectedUser.user.id}`, values);
  ...
  const handleCreateUser = async (values) => {
    setLoading(true);
    try {
      await axios.post("http://3.22.98.109:8080/api/createusers", {
        ...values,
        address: " ",
        neighborhood: " ",
      });
    ...
    try {
      ...
      const responses = await Promise.all(
        filteredOrders.map(async (order) => {
          try {
            const response = await axios.get(`http://3.22.98.109:8080/api/orders/${order.id}`);
