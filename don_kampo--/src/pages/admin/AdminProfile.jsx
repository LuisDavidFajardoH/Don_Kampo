...
const AdminProfile = () => {
  ...
  const sendProductsToAPI = async (products) => {
    try {
      for (const product of products) {
        ...
        await axios.post("https://api.donkampo.com/api/createproduct", formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      }
  ...
  const fetchShippingCosts = async () => {
    try {
      const response = await axios.get("https://api.donkampo.com/api/customer-types");
  ...
  const updateShippingCosts = async (values) => {
    ...
    try {
      await axios.put("https://api.donkampo.com/api/customer-types/shipping-costs", values);
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get("https://api.donkampo.com/api/users");
  ...
  const fetchOrders = async () => {
    try {
      const response = await axios.get("https://api.donkampo.com/api/orders");
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      ...
      await axios.put(`https://api.donkampo.com/api/updatestatus/${orderId}/${newStatus}`);
  ...
  const deleteOrder = async (orderId) => {
    try {
      await axios.delete(`https://api.donkampo.com/api/deleteorders/${orderId}`);
  ...
  const openUserModal = async (user) => {
    try {
      const response = await axios.get(`https://api.donkampo.com/api/users/${user.id}`);
  ...
  const updateUserDetails = async (values) => {
    try {
      await axios.put(`https://api.donkampo.com/api/updateusers/${selectedUser.user.id}`, values);
  ...
  const handleCreateUser = async (values) => {
    setLoading(true);
    try {
      await axios.post("https://api.donkampo.com/api/createusers", {
        ...values,
        address: " ",
        neighborhood: " ",
      });
    ...
    try {
      ...
      const responses = await Promise.all(
        filteredOrders.map(async (order) => {
          try {
            const response = await axios.get(`https://api.donkampo.com/api/orders/${order.id}`);
